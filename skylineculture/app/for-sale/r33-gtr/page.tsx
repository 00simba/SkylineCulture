...
export default function ListingsPage() {
  ...
  const r33GtrJsonLd = generateCollectionJsonLd("R33 GT-R For Sale", cars.map((c) => ({
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {cars.map((car) => (
            ...
            <Link
              key={car.id}
              href={`/for-sale/r33-gtr/${car.year}-${car.color.toLowerCase()}-${car.trim.toLowerCase()}-${car.id.toLowerCase()}`}
